# app.py - 통합된 Rallit 스마트 채용 대시보드 (기능 개선 완료 버전)
...
def render_smart_matching(filtered_df, user_profile, matching_engine, all_df):
    ...
    # (2) "데이터 없음" 상황에 대한 대안 제안
    if not match_results:
        st.warning("아쉽지만, 현재 필터 조건에 맞는 추천 공고가 없습니다. 필터를 조정해보세요.")
        with st.expander("🤔 혹시 이런 건 어떠세요?"):
            st.markdown("**다른 직무 찾아보기**")
            other_categories = [cat for cat in all_df['job_category'].unique() if cat != filtered_df['job_category'].iloc[0] if not filtered_df.empty]
            st.write(f"현재 직무 외에도 이런 직무들이 있습니다: `{'`, `'.join(other_categories[:3])}`")
            
            st.markdown("**인접 기술 스택 학습하기**")
            adjacent_skills = {'React': 'Vue.js', 'Python': 'Go', 'AWS': 'GCP, Azure', 'Docker': 'Kubernetes'}
            suggestions = [f"`{v}`" for k, v in adjacent_skills.items() if k in user_profile['skills']]
            if suggestions:
                st.write(f"현재 보유 스킬 기반으로 이런 기술을 추가 학습하면 좋습니다: {', '.join(suggestions)}")
        return
    ...
    for i, res in enumerate(sorted(match_results, key=lambda x: x['score'], reverse=True)[:5]):
        with st.expander(f"🏆 #{i+1} {res['title']} - 매칭도: {res['score']:.1f}%"):
            ...
            with c1:
                ...
                if res['matched']: st.markdown("**🎯 보유 스킬 매치:**" + "".join([f'<div class="skill-match">✅ {s}</div>' for s in res['matched']]), unsafe_allow_html=True)
                if res['missing']: st.markdown("**📚 추가 학습 필요:**" + "".join([f'<div class="skill-gap">📖 {s}</div>' for s in res['missing'][:3]]), unsafe_allow_html=True)
            with c2:
...
def main():
    ...
    filtered_df = df[(df['job_category'] == user_category if user_category != '전체' else True) & (df['address_region'] == selected_region if selected_region != '전체' else True) & (df['join_reward'] > 0 if reward_filter else True) & (df['is_partner'] == 1 if partner_filter else True) & (df['job_level'].isin(selected_levels)) & (df['join_reward'].between(join_reward_range[0], join_reward_range[1]))]
    if keyword_input: filtered_df = filtered_df[filtered_df.apply(lambda row: keyword_input.lower() in str(row['title']).lower() or keyword_input.lower() in str(row['company_name']).lower(), axis=1)]
    ...
    st.success(f"🔍 **필터 요약:** {active_filters if active_filters else '전체'} | **결과:** `{len(filtered_df)}`개의 공고")
