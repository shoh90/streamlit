# app.py - 단일 파일 통합 버전 (시각화 + UX 개선 + 예측 분석 기능)
...
def main():
    ...
    filtered_df = apply_filters(df, user_category, selected_region, reward_filter)

    st.header("🎯 스마트 매칭 결과")
    st.markdown(f"🔍 **현재 필터** → 관심 직무: `{user_category}` / 지역: `{selected_region}` / 기술스택: `{', '.join(user_profile['skills'])}`")
    ...
    if user_skills_input:
        ...
        for _, row in filtered_df.iterrows():
            ...
            growth, _ = matching_engine.analyze_growth_potential(user_profile)
            predict = matching_engine.predict_success_probability(score, growth)
            if score > 20:
                results.append({
                    '공고명': row['title'],
        ...
        if results:
            ...
            st.warning("😥 조건에 맞는 공고가 없습니다. 필터를 완화하거나 스킬을 추가해보세요!")
    else:
        st.info("👈 사이드바에서 기술 스택을 입력해주세요.")

    st.markdown("---")
    st.subheader("🔮 예측 분석 설명")
    st.markdown("""
    - **예측 합격률**은 사용자의 기술 매칭도와 성장 잠재력을 기반으로 추정됩니다.  
    - 추후에는 실제 기업 평가 데이터와 연동하여 정확도를 높일 예정입니다.  
    - 예: 매칭도 70%, 성장 점수 60점 → 예측 합격률 67%
    """)

    with st.sidebar.expander("📂 이력서 업로드 (준비 중)"):
        st.file_uploader("PDF 또는 CSV 형식 이력서를 업로드하세요", type=['pdf', 'csv'])
        st.caption("AI가 이력서를 분석하여 매칭률을 개선할 수 있도록 준비 중입니다.")
