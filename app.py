"""
Rallit Jobs Dashboard - 스마트 기능이 통합된 최종 Streamlit 애플리케이션 (오류 수정 완료)
"""
...
class SmartDataLoader:
    ...
    def load_from_database(_self):
        try:
            ...
            # job_skill_keywords 컬럼이 존재하는지 확인하고, 없으면 추가
            cursor = conn.cursor()
            cursor.execute("PRAGMA table_info(jobs)")
            columns = [info[1] for info in cursor.fetchall()]
            if 'job_skill_keywords' not in columns:
                cursor.execute("ALTER TABLE jobs ADD COLUMN job_skill_keywords TEXT")
        ...
        try:
            ...
            for category, filename in self.csv_files.items():
                ...
                if file_path.exists():
                    ...
                    df_cat['job_category'] = category
                    all_dfs.append(df_cat)
            if all_dfs:
                combined_df = pd.concat(all_dfs, ignore_index=True)
                combined_df.columns = [col.lower().replace(' ', '_') for col in combined_df.columns]
                # 'join_reward'와 같은 숫자형 컬럼이 object 타입일 경우 숫자로 변환
                if 'join_reward' in combined_df.columns:
                    combined_df['join_reward'] = pd.to_numeric(combined_df['join_reward'], errors='coerce').fillna(0)
...
def main():
        ...
        if user_skills_input:
            ...
            for idx, job in filtered_df.iterrows():
                ...
                if match_score > 20: # 최소 매칭 점수
                    matching_results.append({'idx': idx, 'title': job['title'], 'company': job['company_name'], 'score': match_score, 'matched': matched, 'missing': missing})
            ...
            for i, res in enumerate(matching_results[:5]):
                with st.expander(f"🏆 #{i+1} {res['title']} - 매칭도: {res['score']:.1f}%"):
                    ...
                    with col2:
                        ...
                        st.plotly_chart(fig, use_container_width=True, key=f"match_gauge_{res['idx']}_{i}")
    ...
    with tabs[1]:
        ...
        if 'job_skill_keywords' in filtered_df.columns:
            ...
            st.warning("기술 스택 정보(job_skill_keywords)가 데이터에 없습니다.")


    with tabs[2]:
        st.header("📈 개인 성장 경로 분석")
        if user_skills_input and 'job_skill_keywords' in df.columns:
            ...
            if not req_counts.empty:
                ...
                st.info("선택된 직무에 대한 스킬 요구사항 데이터가 부족합니다.")
        else:
    ...
    with tabs[5]:
        ...
        st.download_button("📄 CSV 다운로드", data=csv, file_name="rallit_jobs.csv", mime="text/csv")
