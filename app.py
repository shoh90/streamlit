# app.py - 통합된 Rallit 스마트 채용 대시보드 (단일 파일, 헤더 수정 완료)
...
st.markdown("""
<style>
    /* 헤더 스타일은 삭제하고, 카드 및 스킬 태그 스타일만 유지 */
    ...
    .skill-match { display: inline-block; background: #e8f5e8; padding: 0.3rem 0.6rem; border-radius: 5px; border-left: 3px solid #4caf50; margin: 0.2rem; font-size: 0.9em; }
    .skill-gap { display: inline-block; background: #fff3e0; padding: 0.3rem 0.6rem; border-radius: 5px; border-left: 3px solid #ff9800; margin: 0.2rem; font-size: 0.9em;}
...
class SmartDataLoader:
    ...
    def load_from_database(_self):
        try:
            if not Path(_self.db_path).exists():
                logger.warning("DB not found. Creating from CSV.")
    ...
    def _load_from_csv_fallback(self):
        try:
            ...
            df.columns = [c.lower().replace(' ', '_').replace('.', '_') for c in df.columns] # 컬럼명 클리닝 강화
            for col in ['join_reward', 'is_partner', 'is_bookmarked']:
                if col in df.columns:
                    df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)
            return df
...
class SmartMatchingEngine:
    ...
    def analyze_growth_potential(self, user_profile):
        ...
        if any(skill in modern_skills for skill in [s.lower() for s in user_profile.get('skills', [])]):
            score += 20; factors.append("최신 기술 트렌드 관심")
...
def render_smart_matching(filtered_df, user_profile, matching_engine):
    ...
    if not user_profile['skills']:
        st.info("👆 사이드바에 보유 기술을 입력하면 맞춤 공고를 추천해 드립니다."); return
    ...
    if not match_results:
        st.warning("아쉽지만, 현재 필터 조건에 맞는 추천 공고가 없습니다. 필터를 조정해보세요."); return
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.header("🔍 고급 필터")
        job_categories = ['전체'] + sorted(list(df['job_category'].dropna().unique()))
        user_category = st.selectbox("관심 직무", job_categories)
        selected_region = st.selectbox("📍 근무 지역", ['전체'] + sorted(list(df['address_region'].dropna().unique())))
        ...
        selected_levels = st.multiselect("📈 직무 레벨", df['job_level'].dropna().unique(), default=list(df['job_level'].dropna().unique()))
    ...
    with tabs[4]: st.header("🔮 예측 분석"); st.info("AI 기반 예측 기능은 곧 출시될 예정입니다. 🚀")
